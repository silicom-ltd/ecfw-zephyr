...
K_THREAD_DEFINE(smchost_thrd_id, EC_TASK_STACK_SIZE, smchost_thread,
	...
	defined(CONFIG_THERMAL_MANAGEMENT_V4) || \
	defined(CONFIG_THERMAL_MANAGEMENT_V5)
/* 1s, tuned for V4; shared by V1-V3/V5 too (add a per-version period
 * config in the future if needed)
 */
const uint32_t thermal_thrd_period = 1000;
